function [nll, grad] = bivariateINGARCHnllM3(theta, data)
    omega1 = theta(1);
    alpha11 = theta(2);
    beta11 = theta(3);
    beta12 = theta(4);
    ...
    for t = 2:n
        lambda1(t) = omega1 + alpha11 * lambda1(t-1) + beta11 * data(t-1, 1) + beta12 * data(t-1, 2);
        lambda2(t) = omega2 + alpha22 * lambda2(t-1) + beta21 * data(t-1, 1) + beta22 * data(t-1, 2);
        ...
        d_lambda1_d_beta12(t) = data(t-1, 2) + alpha11 * d_lambda1_d_beta12(t-1);
        d_lambda2_d_omega2(t) = 1 + alpha22 * d_lambda2_d_omega2(t-1);
        d_lambda2_d_alpha22(t) = lambda2(t-1) + alpha22 * d_lambda2_d_alpha22(t-1);
        d_lambda2_d_beta21(t) = data(t-1, 1) + alpha22 * d_lambda2_d_beta21(t-1);
    ...
    for t = 1:n
        ...
        grad(3) = grad(3) + (term1 * d_lambda1_d_beta11(t));
        grad(4) = grad(4) + (term1 * d_lambda1_d_beta12(t));
        % Gradient w.r.t theta_2
        grad(5) = grad(5) + (term2 * d_lambda2_d_omega2(t));
        grad(6) = grad(6) + (term2 * d_lambda2_d_alpha22(t));
        grad(7) = grad(7) + (term2 * d_lambda2_d_beta21(t));
